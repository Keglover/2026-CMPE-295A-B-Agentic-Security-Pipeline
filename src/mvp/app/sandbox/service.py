...
def _effective_container_runtime(container: object) -> str:
    """Best-effort lookup of the effective runtime used by a spawned container."""
    try:
        container.reload()
        host_cfg = container.attrs.get("HostConfig", {})
        runtime = str(host_cfg.get("Runtime") or "")
        if runtime:
            return runtime
        return "docker-default"
    except Exception as exc:
        _log.warning("Could not inspect effective container runtime: %s", exc)
        return "unknown"
...
def execute_tool(tool_name: str, payload: ExecuteRequest) -> dict:
    ...
    try:
        ...
        if not isinstance(container, (bytes, bytearray)):
            runtime_effective = _effective_container_runtime(container)
            _log.info(
                "Runtime evidence: job_id=%s container_id=%s requested_runtime=%s effective_runtime=%s",
                job_id,
                container.id[:12],
                runtime or "docker-default",
                runtime_effective,
            )
            if runtime_effective == "runsc":
                _log.info(
                    "gVisor active for job_id=%s (runtime=runsc). "
                    "Syscalls are mediated by gVisor's user-space Sentry.",
                    job_id,
                )
            elif USE_GVISOR:
                _log.warning(
                    "USE_GVISOR=true but effective runtime is '%s' for job_id=%s",
                    runtime_effective,
                    job_id,
                )
